...
void callback_goal_reached(const nav2d_navigator::MoveToPosition2DActionResult::ConstPtr& msg)
{
...
{    
    std::cout << "INITIALIZING TEST ONCE OF PATH PLANNING ALGORITHMS USE nav2d'S SOFTWARE..." << std::endl;
    ...
    ros::Publisher  pubGoalPoint =   n.advertise<geometry_msgs::PoseStamped>("/goal", 1);
    ros::Subscriber subGoalReachec = n.subscribe("/MoveTo/result", 1, callback_goal_reached);
	...
	case SM_SEND_TO_GOAL:
	    ...
	    goalPose.pose.position.x = goal_x[current_goal];
	    goalPose.pose.position.y = goal_y[current_goal];
	    pubGoalPoint.publish(goalPose);
	...
	case SM_WAIT_FOR_GOAL_REACHED:
	    ...
	    travel_distances[current_goal] += delta_d;
            ...
            if(global_goal_reached) //Flag is changed in the callback
	    {
