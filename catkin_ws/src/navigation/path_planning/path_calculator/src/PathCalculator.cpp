                         ...
                         nav_msgs::Path& resultPath)
{
    ...
    while(currentCell != goalCell && !fail && attempts < map.data.size())
    {
	    ...
	    //for 8 connectivity, h_value must be euclidean distance
	    int dist_to_goal_x = ((neighbors[i]%map.info.width) - goalCellX);
	    int dist_to_goal_y = ((neighbors[i]/map.info.width) - goalCellY);
	    int h_value = (int)(sqrt(dist_to_goal_x*dist_to_goal_x + dist_to_goal_y*dist_to_goal_y));
